...
export class Config {
  ...
  public async getProduct(): Promise<Product[]> {
    const data: Product[] = await fetch(this.apiURL + "/product").then(
      (response) => {
        if (!response.ok) {
  ...
  public async updateProduct(product: Omit<Product, "id">) {
    const result: { generatedMaps: []; raw: []; affected: number } =
      await fetch(this.apiURL + `/product/${product.code}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  public async deleteProduct(code: string) {
    const result: { raw: []; affected: number } = await fetch(
