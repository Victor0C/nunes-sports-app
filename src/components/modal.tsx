import { Button, FormControl, FormLabel, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalHeader, ModalOverlay, useDisclosure, useToast } from "@chakra-ui/react"
...
export function ModalProduct(props: PropsModal){
    ...
    return(
        <>
           {
            props.button ? (<Button colorScheme='blue' onClick={onOpen} className="mt-1">Cadastrar</Button>) : null
           }
            <Modal isOpen={!props.editModal? isOpen : props.editModal.isOpen} onClose={!props.editModal? onClose : props.editModal.onClose}>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>
                        {!props.editModal? 'Cadastrar novo produto' : `Editar produto ${props.editModal.initialDdata?.name} (${props.editModal.initialDdata?.code})`}
                    </ModalHeader>
                        <ModalCloseButton/>
                        <ModalBody>
                            <form onSubmit={handleSubmit(!props.editModal? formNewProduct : formUpdateProduct)}>
                                <FormControl isRequired className="d-grid gap-2">
                                    ...
                                    <Controller
                                        ...
                                        {...field}
                                         ...
                                         onChange={(e) => e.target.value.length <= 9 && field.onChange(e)}
                                    ...
                                    <Controller
                                        ...
                                        value={props.editModal?.initialDdata?.
                                        code} 
                                    ...
                                    <Button variant="ghost" mr={3} onClick={() => !props.editModal? onClose() : props.editModal.onClose()}>
                                        Cancelar
                                    </Button>
